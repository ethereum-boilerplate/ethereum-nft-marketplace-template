...
const NFTLister: React.FC = ({ nft, web3, marketplaceAddress }) => {
    ...
    return (
        ...
        isVisible={true}
        okText={"List NFT"}
        onOk={() => listNFT(nft.token_address, nft.token_id, currency ? currency : "0x0000000000000000000000000000000000000000", price, 1, 1, 0, 0, account)}
        isOkDisabled={!price}
        children={[
            <div key={1} style={{display: "grid", gap: "30px", placeItems: 'center'}}>
                <div style={{ maxWidth: "250px", maxHeight: "250px", borderRadius: '15px', overflow: "hidden", display: "grid", placeItems: "center"}}>
                    {
                        // @ts-ignore
                ...
                <div style={{width: "100%", display: 'flex', gap: '10px'}}>
                    <Select
                        ...
                        label={"Currency"}
                        onChange={(e) => {setCurrency(e.tokenAddress)}}
                        options={[
                            {
                    ...
                    />
                    <Input width={"70%"} onChange={(e) => (e as any).target.value > 0 ? setPrice(Moralis.Units.ETH(String((e as any).target.value))) : null} label={"Price"} type={"number"} />
                </div>
            </div>
        ]}
        />
